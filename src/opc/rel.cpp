...
Relationship::Relationship(const QString &rId, const QString &reltype,
                           const QString &targetRef, Part *target,
                           const QString &baseURI, bool external)
    : m_rId(rId),
...
Relationship *Relationships::addRelationship(const QString &reltype,
                                             const QString &targetRef,
                                             Part *target, const QString &rId,
                                             bool external) {
  Relationship *rel =
      new Relationship(rId, reltype, targetRef, target, m_baseURI, external);
  m_rels[rId] = rel;
...
Relationship *Relationships::getOrAddPart(const QString &reltype, Part *target,
  ...
  if (rel) return rel;
  QString rId = nextrId();
  return addRelationship(reltype, target->partName().relativeRef(baseUri),
                         target, rId);
}
 ...
 * \return
 */
Relationship *Relationships::getOrAddExtPart(const QString &reltype,
                                             const QString &targetref) {
...
Relationship *Relationships::getMatchingExt(const QString &reltype,
                                            const QString &targetref) {
  for (Relationship *rel : m_rels.values()) {
