...
void Package::loadRel(const QString &reltype, const QString &targetRef,
                      Part *target, const QString rId, bool isternal) {
...
ImageParts *Package::imageparts() const { return m_imageParts; }

Package::~Package() { delete m_imageParts; }
...
void ImageParts::append(ImagePart *item) {
  if (m_imageparts.contains(item)) return;
