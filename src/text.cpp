 ...
 * \return
 */
Paragraph *Paragraph::insertParagraphBefore(const QString &text,
                                            const QString &style) {
...
void Run::addBreak(WD_BREAK breakType) {
  ...
  if (!eletype.isEmpty()) brele.setAttribute(QStringLiteral("w:type"), eletype);
 ...
 * \return
 */
InlineShape *Run::addPicture(const QString &path, const Length &width,
                             const Length &height) {
...
InlineShape *Run::scalePicture(InlineShape *picture, const Length &width,
                               const Length &height) {
...
InlineShape *Run::addPicture(const QImage &img, const Length &width,
                             const Length &height) {
  InlineShapes *ships = m_part->m_inlineshapes;
